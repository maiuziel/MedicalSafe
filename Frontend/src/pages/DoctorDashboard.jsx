...
const [messageAlert, setMessageAlert] = useState("");
  ...
  const fetchAppointments = async (isFirstLoad = false) => {
    try {
      ...
      const res = await fetch("https://medicalsafe-backend.onrender.com/api/doctor/appointments", {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const handleNotificationClick = async (n) => {
    try {
      ...
      // 🔹 סימון כנקרא
      await fetch(
        `https://medicalsafe-backend.onrender.com/api/notifications/${n._id}/read`,
        {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${token}`,
...
setTimeout(() => setMessageAlert(""), 3000);
  ...
  const updateStatus = async (status) => {
    try {
      ...
      await fetch(
        `https://medicalsafe-backend.onrender.com/api/doctor/requests/${selectedRequest._id}/status`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const updateAppointmentStatus = async (id, status) => {
    try {
      ...
      const res = await fetch(
        `https://medicalsafe-backend.onrender.com/api/doctor/appointments/${id}/status`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
