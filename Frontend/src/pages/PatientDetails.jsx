  ...
  const fetchData = async () => {
    try {
      ...
      const resPatient = await fetch(
        `https://medicalsafe.duckdns.org/api/patient/${id}`,
        {
          headers: {
      ...
      const resAppointments = await fetch(
        `https://medicalsafe.duckdns.org/api/appointments/patient/${id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      const resRecords = await fetch(
        `https://medicalsafe.duckdns.org/api/medical-records/patient/${id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
...
setMedicalRecords(dataRecords);
  ...
  const fetchFollowUpLogs = async () => {
    try {
      ...
      const res = await fetch(
        `https://medicalsafe.duckdns.org/api/follow-up/logs/${id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const toggleFollowUp = async () => {
    try {
      ...
      const res = await fetch(
        `https://medicalsafe.duckdns.org/api/follow-up/${id}`,
        {
          method,
          headers: {
  ...
  const fetchFiles = async () => {
    try {
      ...
      const res = await fetch(
        `https://medicalsafe.duckdns.org/api/doctor/patient/${id}/files`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const handleSaveEdit = async () => {
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(`https://medicalsafe.duckdns.org/api/medical-records/${editingRecord}`, {
        method: "PUT",
          ...
          {files.length === 0 ? (
            ...
            files.map((f) => (
              ...
              >
                ...
                <a
                  href={`https://medicalsafe.duckdns.org/uploads/${f.file}`}
                  target="_blank"
                  rel="noreferrer"
                  style={{ color: "blue" }}
                >
