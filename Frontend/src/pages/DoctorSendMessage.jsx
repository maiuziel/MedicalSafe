...
export default function DoctorSendMessage() {
  ...
  const fetchSecretaries = async () => {
    try {
      const res = await fetch("http://62.238.31.43:3000/api/messages/secretaries", {
  ...
  const fetchHistory = async () => {
    const res = await fetch("http://62.238.31.43:3000/api/messages/sent", {
      headers: { Authorization: `Bearer ${token}` },
    });
  ...
  const handleSend = async () => {
    ...
    const res = await fetch("http://62.238.31.43:3000/api/messages/doctor-to-secretary", {
      method: "POST",
      headers: {
